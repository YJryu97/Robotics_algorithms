...
def train_model(policy, baseline, trajs, policy_optim, baseline_optim, gamma=0.99, baseline_train_batch_size=64, baseline_num_epochs=5):
    ...
    for traj in trajs:
        ...
        # TODO start
        current_return = 0
        for i in np.flip(range(len(returns_singletraj))):
            # discounted sum of rewards from that timestep to the end
            current_return = rewards_singletraj[i] + gamma*current_return 
            returns_singletraj[i] = current_return   
        # TODO end
    ...
    for epoch in range(baseline_num_epochs):
        ...
        for i in range(n // baseline_train_batch_size):
            ...
            # TODO start         
            predicted_returns = baseline(torch.Tensor(states).to(device)[batch_index])
            loss = criterion(predicted_returns, torch.Tensor(returns).to(device)[batch_index])
            # TODO end
    ...
    baseline_pred = baseline(torch.from_numpy(states).float().to(device)) # final baseline
    # TODO start
    loss = (-log_policy * (torch.Tensor(returns).to(device) - baseline_pred)).mean()
    # TODO end
...
def simulate_policy_pg(env, policy, baseline, num_epochs=20000, max_path_length=50, pg_batch_size=100, 
    ...
    for iter_num in range(num_epochs):
        ...
        # Training model
        loss = train_model(policy, baseline, sample_trajs, policy_optim, baseline_optim, gamma=gamma, 
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('Training Loss - Policy Gradient')
    plt.savefig('training_loss_plot_pg.png')