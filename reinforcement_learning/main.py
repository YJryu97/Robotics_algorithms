...
if __name__ == '__main__':
    ...
    if args.task == 'behavior_cloning':
        ...
        # Train behavior cloning
            ...
            """for i, el in enumerate(episode_length):
                losses = simulate_policy_bc(env, policy, expert_data, num_epochs=num_epochs, episode_length=el,
                            batch_size=batch_size)
                plt.plot(losses, label=f'Episode Length: {el}')

            plt.xlabel('Epoch')
            plt.ylabel('Loss')
            plt.title('Agent Performance with Different Episode Lengths')
            plt.legend()
            plt.savefig('peform_loss_plot_bc.png')"""
        else:
